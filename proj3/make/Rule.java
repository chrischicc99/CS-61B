...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("Adding command to exist rule");
        }
    ...
    private void checkFinishedDependencies() {
        Iteration<Integer> successors = _depends.successors(_vertex);
        for (Integer depends : successors) {
            Rule rule = _depends.getLabel(depends);
            if (rule == null || rule.isUnfinished()) {
                throw new IllegalStateException("Unfinished dependencies "
                        + depends);
            }
        }
    }
